...
def test_create(runner):
    result = runner.invoke(habit.habit, ['create'], obj={'session_maker': session_maker}, input='Test Habit\nd\n')
    assert 'Habit "Test Habit" has been created with a Daily Periodicity!' in result.output


def test_complete(runner):
    result = runner.invoke(habit.habit, ['complete'], obj={'session_maker': session_maker}, input='Test Habit')
    assert 'You have completed Habit "Test Habit"! (Streak: 1)' in result.output


def test_modify(runner):
    result = runner.invoke(habit.habit, ['modify'], obj={'session_maker': session_maker}, input='1\nY\nChanged Habit\nN')
    assert 'Habit has been updated!' in result.output


def test_delete(runner):
    result = runner.invoke(habit.habit, ['delete'], obj={'session_maker': session_maker}, input='Changed Habit\nY')
